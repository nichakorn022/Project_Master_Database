...
<main class="flex-1 bg-gray-50 dark:bg-gray-900" x-data="backstampPage()" x-init="initSelect2()">
        ...
        <div class="dark:bg-gray-800 dark:shadow-gray-900/50 bg-white p-6 rounded-lg shadow-md mb-3">
            <form method="GET" action="{{ route('backstamp.index') }}" class="space-y-4">
                ...
                <div x-show="showFilter" x-cloak class="border border-gray-200 rounded-lg p-4 bg-gray-50 dark:bg-gray-700/40 dark:border-gray-600">
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <div>
                            <label for="organic" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('content.organic') }}</label>
                            <select id="organic" name="organic" onchange="this.form.submit()"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100">
                                <option value="">All</option>
                                <option value="1" {{ $organic === '1' ? 'selected' : '' }}>Yes</option>
        ...
        @if ($customerId || $requestorId || $statusId || ($organic !== null && $organic !== '') || ($exclusive !== null && $exclusive !== ''))
            <div class="flex flex-wrap gap-2 mb-3">
                ...
                @if ($customerId)
                    <a href="{{ route('backstamp.index', array_merge(request()->except('customer_id', 'page'), ['per_page' => request('per_page', 10)])) }}"
                ...
                @endif
                @if ($requestorId)
                    <a href="{{ route('backstamp.index', array_merge(request()->except('requestor_id', 'page'), ['per_page' => request('per_page', 10)])) }}"
                ...
                @endif
                @if ($statusId)
                    <a href="{{ route('backstamp.index', array_merge(request()->except('status_id', 'page'), ['status_id' => 'all', 'per_page' => request('per_page', 10)])) }}"
                ...
                @endif
                @if ($organic !== null && $organic !== '')
                    <a href="{{ route('backstamp.index', array_merge(request()->except('organic', 'page'), ['per_page' => request('per_page', 10)])) }}"
                        ...
                        <span>{{ __('content.organic') }}: {{ $organic === '1' ? 'Yes' : 'No' }}</span>
                        <span class="material-symbols-outlined text-[8px]">close</span>
                    </a>
                @endif
